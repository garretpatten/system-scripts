...
get_default_branch() {
    ...
    # If all else fails, return 1 (failure)
    return 1
}
...
sync_repo() {
    ...
    echo -e \"--- Processing: \$repo_name ---\" # Always print separator
...
main() {
    ...
    # Use -prune to avoid searching inside .git directories themselves
    find "$search_dir" -name ".git" -type d -prune | while read -r git_dir; do
        local repo_path; repo_path="$(dirname "$git_dir")"
        local result_output
